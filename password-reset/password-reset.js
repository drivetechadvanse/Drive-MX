...
const waitForFirebaseConfig = () => new Promise((resolve, reject) => {
...
const getPasswordResetAuth = async () => {
...
const getFirebaseErrorMessage = (code = '') => {
    const messages = {
        ...
        'auth/api-key-not-valid.-please-pass-a-valid-api-key.': 'La configuración de Firebase no está lista. Actualiza la página e intenta nuevamente.',
        'auth/invalid-api-key': 'La configuración de Firebase no está lista. Actualiza la página e intenta nuevamente.',
        'auth/app-not-authorized': 'El dominio actual no está autorizado en Firebase Authentication.',
        'auth/operation-not-allowed': 'La recuperación por correo no está habilitada en Firebase Authentication.'
    };
...
const createModal = () => {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            const auth = await getPasswordResetAuth();
