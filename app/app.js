...
const App = () => {
    ...
    const isSupermarketOnlyPurchase = (productsToValidate = checkoutProducts) => {
        const items = Array.isArray(productsToValidate) ? productsToValidate.filter(Boolean) : [];
        return items.length > 0 && items.every(product => isSupermarketPurchaseProduct(product));
    };
    ...
    const startCartCheckout = () => {
        ...
        if (!isSupermarketOnlyPurchase(validCartProducts) && !ensureCheckoutWalletsAllowed(validCartProducts)) return;
...
Comunícate al 5633535701 o 5617549756 para la recolección de tu paquete.`,
    ...
    const payWithWallet = async () => {
        ...
        if (!ensureSupermarketMinimumAllowed(checkoutProducts)) return;
        if (!ensureCheckoutInventoryAllowed(checkoutProducts)) return;
        if (!isSupermarketOnlyPurchase(checkoutProducts) && !ensureCheckoutWalletsAllowed(checkoutProducts)) return;
